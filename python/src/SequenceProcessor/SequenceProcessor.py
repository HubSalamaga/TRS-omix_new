...
class SequenceProcessor:
    ...
    def filter_taxonomy_file(taxonomy_file,accessions,chunksize= 50000):
        ...
        try:
            ...
            for chunk in tqdm(pd.read_csv(taxonomy_file,sep="\t",usecols=cols_to_use, chunksize= chunk), total= estimated_chunks):
                filtered_chunk = chunk[chunk.ilocp[:,0].isin(accessions)]
    ...
    @staticmethod
    def ask_for_exceptions(): #NEEDS TO BE REWRITTEN TO ACCEPT THE ARGUMENTS 
        """
        Asks user for exceptions to further filtering steps.
        """
        exceptions = set()
        while True:
            user_input = input("Would you like to add exceptions to further filtering steps (e.g., top-level species)? (y/file/n): ")
            if user_input.lower() == 'y':
                print("Enter a taxid, a list of comma-separated taxids")
                taxid_input = input("Enter your choice here: ")
                exceptions = SequenceProcessor.process_taxid_input(taxid_input)
                break
                ...
                while True:
                    ...
                    file_path = FileHandler.find_file_by_name(file_path, auto=True)
                    try:
                        ...
                        if input("Type 'continue' to try again or 'exit' to stop: ").lower() == 'exit':
                            return
                    except Exception as e:
                        print(f"An unexpected error has occurred: {e}")
                        return
